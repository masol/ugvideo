...
"${envImageType}, standard 16:9 widescreen composition, ultra wide establishing shot, deep focus, deep depth of field, foreground mid-ground background layered composition with strong spatial depth, architectural photography, location scouting reference, ${envStyleAnchor}, [空间描述], [固定陈设仅作空间锚点], [光源来自空间自身], [现实参照], [光照方案], [整体氛围（与场景氛围基调一致）], [风格收尾]"`,
    ...
    user: (params: {
        ...
        prompt += `【场景原文（仅用于推断空间类型/现实案例参照/光源位置/整体氛围，不从中提取人物活动）】\n${params.sceneText.slice(0, 500)}\n\n`;
...
**输出格式**：直接输出细化后的完整提示词（保留初次设计的锚定模板开头，**禁止修改**模板段）。`,
    ...
    user: (params: {
        ...
        return `【场景 ${params.sceneId}】\n\n` +
            `【初次设计】\n${params.designPrompt}\n\n` +
            `【场景氛围/剧情上下文（细化方向必须与此氛围一致；整洁明亮的场景不得被细化为破败废墟）】\n${params.moodContext}\n\n` +
            `请输出深化后的完整环境基底图提示词。保留初次设计的锚定模板开头不要修改。细化的方向和密度必须与【场景氛围基调】一致。`;
    },
};
...
**判定原则**：宁可放过，不可错杀。仅在"明显导致镜头无法拍摄"或"明显与剧情/氛围矛盾"时判 REVISE。
